...
const client = new OpenAI({
  apiKey: process.env.DASHSCOPE_API_KEY || 'sk-4561661278024ff4bcd1844225834ea8',
...
export async function POST(request: NextRequest) {
  try {
    const { styleAnalysis, topic, framework } = await request.json()
    ...
    const prompt = `你是一位真实的视频内容创作者，正在为自己的新视频撰写文案。你要像真人一样写作，而不是像AI一样套模板。
    ...
    const completion = await withRetry(async () => {
      return await client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: '你是一位真实的视频内容创作者，有着丰富的创作经验。你擅长像真人一样写作，用接地气的方式表达观点，避免AI式的套路和过度比喻。你的文案让人感觉是朋友在聊天，而不是在看一篇精心包装的文章。'
          },
          {
            role: 'user',
            content: prompt
